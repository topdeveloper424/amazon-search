<template>
  <div class="save-search">
    ...
    <b-row class="mt-3">
      <b-col cols="6" class="text-right" offset="1">
      ...
      <b-col cols="2" class="text-left">
          <b-button variant="primary" size="sm" id="searchBtn" @click="search"><b-icon icon="search" class="mr-1" ></b-icon>Search</b-button>
      </b-col>
    ...
    <b-row class="mt-3">
      <b-col cols="5" class="text-left">
        <select v-model="currentSearch" class="form-control" id="searchSelect" size="sm" @change="changeSearch()">
          <option v-for="searchItem in savedSearches" :key="searchItem._id" v-bind:value="searchItem._id">{{searchItem.searchName}}</option>
        </select>
      </b-col>
      <b-col cols="2">
          <b-button v-b-modal.save_modal variant="primary" size="sm">Save Search</b-button>
      </b-col>
      <b-col cols="3">
          <b-button variant="success" @click="updateSearch()" size="sm">Update Search</b-button>
      </b-col>
      <b-col cols="2">
          <b-button variant="danger" size="sm" @click="deleteSearch()">Delete Search</b-button>
      </b-col>
    </b-row>
              ...
              <b-card-body>
                <b-card-text class="text-left">
                  <b-row>
                    ...
                    <b-col cols="2">
                      <b-form-input type="number" id="rankInput1" size="sm" v-model="filters.rankNumber1"></b-form-input>
                    </b-col>
                    <b-col cols="1">
                      <
                    </b-col>
                    <b-col cols="2">
                      <b-form-input type="number" id="rankInput2" size="sm" v-model="filters.rankNumber2"></b-form-input>
                    </b-col>
                  </b-row>
                </b-card-text>
  ...
  methods: {
    ...
    async deleteSearch(){
      if(this.currentSearch.trim() === ""){
        ...
        this.$bvModal.msgBoxConfirm('Please confirm that you want to delete this search.', {
          ...
          .then(async (value)=>  {
            if(value === true){
              await this.$store.dispatch('deleteSearchById', {"id":this.currentSearch})
            }
          })
          .catch(err => {
            // An error occurred
    ...
    async updateSearch(){
      if(this.currentSearch.trim() === ""){
        this.$bvModal.msgBoxOk('Please select search to update!')
         ...
         let updateSearchParams = this.$store.state.searchParams;
         updateSearchParams.id = this.currentSearch;
        await this.$store.dispatch('updateSearchById', updateSearchParams)
        this.$bvModal.msgBoxOk('Search was updated successfully', {
          title: 'Success',
          size: 'sm',
          buttonSize: 'sm',
          okVariant: 'success',
